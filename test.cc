...
namespace Apnea
{
  ...
  private:
    ...
    // Initiate the right_hand_side vrector (represente the force)
  template <int dim>
  ...
  template <int dim>
  void water_pressure(const std::vector<Point<dim>> &points,
                       std::vector<Tensor<1, dim>> &  values,
                       double force)
  {
    // Check on the size of the vector values and the vector of points
                    ...
                    if (cell->material_id() == 1)
                    {
                      ...
                      (
                        (fe_values.shape_grad(i, q_point)[component_i] *
                         ...
                         lambda_rib_values[q_point])
                        +
                        (fe_values.shape_grad(i, q_point)[component_j] *
                         fe_values.shape_grad(j, q_point)[component_i] *
                         mu_rib_values[q_point])
                        +
            ...
            if (face->at_boundary())
              {
                ...
                water_pressure(fe_face_values.get_quadrature_points(), pressure_values, force);
  ...
  template <int dim>
  void Apnea_V7<dim>::output_results(double force) const
  {
    std::string filename;
    if (force < 0)
    {
