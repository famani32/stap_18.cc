...
apneaV9<dim>::apneaV9()
    ...
    , timestep_no(0)
{
  this->add_parameter("Finite element degree",fe_degree);
  this->add_parameter("Rhs expression",rhs_expression);
  this->add_parameter("Some constants",constants);//:"pi=3.14,lambda_lung=1794.285714,lambda_rib=923076923.0769",mu_lung=448.571428,mu_rib=4615384615.3846";
  this->add_parameter("Include filename", include_filename);


  rhs=std::make_unique<FunctionParser<3>>(rhs_expression);
...
template <int dim> void apneaV9<dim>::get_grid() {
  ...
  std::string filename = include_filename;
...
template <int dim> void apneaV9<dim>::assemble_system(double force) {
  ...
  for (const auto &cell : dof_handler.active_cell_iterators()) {
    if (cell->is_locally_owned()) {
      ...
      //rhs->value(fe_values.quadrature_point(n_q_points));
     right_hand_side(fe_values.get_quadrature_points(), rhs_values);
    //  rhs_expression(fe_values.get_quadrature_points(), rhs_values);
      ...
      for (const unsigned int i : fe_values.dof_indices()) {
        ...
        for (const unsigned int q_point : fe_values.quadrature_point_indices())
          cell_rhs(i) += fe_values.shape_value(i, q_point) *
                         ...
                         rhs->value(fe_values.quadrature_point(q_point))*
                         fe_values.JxW(q_point);
      }
      for (const auto &face : cell->face_iterators()) {
        if (face->at_boundary()) {
